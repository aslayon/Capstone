...
def log_vehicle_tracking(session_id: str, event_type: str, data: dict, root="tracking_logs"):
    """
    사용자를 위한 차량 추적 로그
    
    Args:
        session_id: 추적 세션 ID (최초 선택 시간 기반)
        event_type: 이벤트 타입 (INITIAL_SELECT, MATCH_FOUND, CAMERA_SWITCH 등)
        data: 로그 데이터 딕셔너리
        root: 로그 루트 디렉터리
    ...
    tracking_logs/
      ├── 2025-01-15/
      │   ├── session_143052_vehicle_track.txt
      │   └── session_150230_vehicle_track.txt
    """
    ...
    if event_type == "INITIAL_SELECT":
        message = f"[추적 시작] 차량 선택 - 카메라: {data.get('camera', '?')}, ID: {data.get('tid', '?')}, 세그먼트: {data.get('segment', '?')}"
    
    elif event_type == "MATCH_FOUND":
        cam_from = data.get('cam_from', '?')
        cam_to = data.get('cam_to', '?')
        tid_from = data.get('tid_from', '?')
        tid_to = data.get('tid_to', '?')
        distance = data.get('distance', 0)
        confidence = data.get('confidence', 'UNKNOWN')
        
        message = (f"[동일 차량 발견] {cam_from}(ID:{tid_from}) → {cam_to}(ID:{tid_to}) "
                  f"| 유사도: {distance:.3f} | 신뢰도: {confidence}")
    
    elif event_type == "CAMERA_SWITCH":
        message = f"[카메라 전환] {data.get('from_cam', '?')} → {data.get('to_cam', '?')}"
...
def run_detect():
    ...
    cfg = load_config()
    global tri_prepare, tracker, tracks, tracks_L, tracks_C, tracks_R, tri_ui_state, selected_bank, track_history_C, track_history_L, track_history_R, switcher, scale, tracking_session_id, tracking_started_time
    tri_prepare = False
    ...
    while True:
        ...
        if click:
            x, y = click
            # 클릭 처리 (기존 mouse_callback과 동일한 로직)
            mouse_callback(cv2.EVENT_LBUTTONDOWN, x, y, 0, None)
...
def mouse_callback(event, x, y, flags, param):
    ...
    if tri_prepare:
        ...
        if clicked_id is not None:
            ...
            update_web_stats(selected_id=clicked_id)
            ...
            update_web_stats(selected_id=None)
            selected_bank.clear()

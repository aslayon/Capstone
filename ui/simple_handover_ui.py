...
class SimpleHandoverUI:
    ...
    def __init__(self):
        ...
        # 현재 표시 해상도(초기: 단일 모드 기준)
        self.display_width = self.display_single_width
        self.display_height = self.display_single_height
    ...
    def set_dual_mode(self, primary_camera: str, secondary_camera: str):
        ...
        # 듀얼 모드 해상도 적용
        self.display_width = self.display_dual_width
        self.display_height = self.display_dual_height
        print(f"듀얼 모드: {primary_camera} + {secondary_camera}")

    def create_display_frame(
        self, frame_dict: Dict[str, np.ndarray]
    ...
    def _create_single_frame(self, frame_dict: Dict) -> Tuple[np.ndarray, Dict]:
        ...
        transform_info = {"mode": "single", "camera": self.primary_camera}
    ...
    def _create_dual_frame(self, frame_dict: Dict) -> Tuple[np.ndarray, Dict]:
        ...
        if self.primary_camera and self.primary_camera in frame_dict:
            ...
            primary_resized = cv2.resize(
                primary_frame, (half_width, self.display_height)
            )
        ...
        if self.secondary_camera and self.secondary_camera in frame_dict:
            ...
            secondary_resized = cv2.resize(
                secondary_frame, (half_width, self.display_height)
            )
        ...
        cv2.line(
            display_frame,
            (half_width, 0),
            (half_width, self.display_height),
            (255, 255, 255),
            4,
        )
    ...
    def _draw_handover_status(self, frame: np.ndarray):
        ...
        overlay = frame.copy()
        cv2.rectangle(
            overlay, (text_x - 20, 80), (text_x + text_size[0] + 20, 130), (0, 0, 0), -1
        )
    ...
    def update_handover_status(
        self, progress: float, message: str = "", elapsed_time: float = 0.0
    ):
        """핸드오버 상태 업데이트"""
    ...
    def handle_click(self, click_x: int, click_y: int) -> Dict:
        ...
        if self.current_mode == UIMode.SINGLE:
            return {
                ...
                "relative_coords": (
                    click_x / self.display_width,
                    click_y / self.display_height,
                ),
            }
        else:  # DUAL
            half_width = self.display_width // 2
